...
def load_limits(limits: dict[str, str], events: dict[str, Event]) \
    -> dict[str, Event]:
    '''Loads the upper limits located within binary NumPy files pointed
    to by the dictionary `limits` generated by `find_limits` and
    returns an updated copy of `events` with the upper limits added.

    Args:
        limits (dict[str, str]): A dictionary containing the identifier
            of each event (as the keys) and the file path to its
            associated binary NumPy file containing upper limits (as
            the values).
        events (dict[str, Event]): A dictionary containing the
            identifier and model of each event (as the keys) and a
            NumPy array with the data type `Event.DTYPE` (as the
    ...
    Returns:
        dict[str, Event]: A dictionary of the same structure as
            `events`, with the upper limits of each event added. If the
            coordinates of an event could not be found by
            `load_places`, the event is excluded.
    '''
